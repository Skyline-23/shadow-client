...
### Current Operator Rules (Required)
- Do not run workspace-wide `xcodebuild clean test` or other whole-repo validation commands unless the user explicitly asks for them or the scoped validation is insufficient.
- Prefer the narrowest validation scope that matches the edited area:
  - `ShadowClientFeatureHome` scheme for Home feature compile gates
  - `ShadowClientmacOS` or `ShadowClientiOSApp` only when platform app wiring changed
  - `cd Modules && swift test` only for touched package modules
- Never run multiple `xcodebuild` commands in parallel against the same workspace/DerivedData path. Validation must be sequential.
- If app-level Swift Testing still reports `Executed 0 tests`, treat that path strictly as a compile gate and rely on scoped Swift Testing/module coverage instead of retrying the full suite.
- When a change affects both iOS and macOS behavior, validate the shared scheme for both destinations separately instead of running a larger aggregate scheme.
