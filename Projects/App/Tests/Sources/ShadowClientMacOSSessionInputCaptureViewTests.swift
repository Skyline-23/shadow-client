...
func macOSInputCaptureDoesNotSyncAbsolutePointerForCapturedButtons() {
    #expect(
        !ShadowClientMacOSPointerInputPolicy.shouldSyncAbsolutePointerBeforeButton(
            isCaptured: true,
            pendingCapturedAnchor: false
        )
    )
...
@Test("macOS input capture emits one absolute anchor when capture first activates")
func macOSInputCaptureAnchorsFirstCapturedMotion() {
    let motion = ShadowClientMacOSPointerInputPolicy.motionEvent(
        locationInView: CGPoint(x: 150, y: 80),
        previousLocationInView: CGPoint(x: 120, y: 90),
        containerBounds: CGRect(x: 0, y: 0, width: 300, height: 200),
...
func macOSInputCaptureSyncsFirstCapturedButtonWithAbsolutePosition() {
    #expect(
        ShadowClientMacOSPointerInputPolicy.shouldSyncAbsolutePointerBeforeButton(
            isCaptured: true,
            pendingCapturedAnchor: true
