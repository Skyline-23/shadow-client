...
@MainActor
func remoteDesktopRuntimeAutomaticallyPushesLocalClipboardChanges() async {
    let metadata = FakeControlTestMetadataClient(
        serverInfoByHost: [
            "192.168.0.28": .init(
                host: "192.168.0.28",
    ...
    let sessionConnector = FakeSessionConnectionClient()
    let clipboard = FakeClipboardClient()
    let runtime = ShadowClientRemoteDesktopRuntime(
        metadataClient: metadata,
        controlClient: control,
        clipboardClient: clipboard,
        sessionConnectionClient: sessionConnector,
        clipboardSyncInterval: .milliseconds(40)
    )
    ...
    await clipboard.setString("auto pushed")
    await waitForClipboardCalls(control, expectedCount: 1)

    let clipboardCalls = await control.clipboardCalls()
    #expect(clipboardCalls.first?.text == "auto pushed")
}

@Test("Remote desktop runtime automatically pulls remote clipboard changes during an active session")
@MainActor
func remoteDesktopRuntimeAutomaticallyPullsRemoteClipboardChanges() async {
    let metadata = FakeControlTestMetadataClient(
        serverInfoByHost: [
            "192.168.0.28": .init(
                host: "192.168.0.28",
    ...
    let control = FakeControlClient(
        simulatedLaunchResult: .init(sessionURL: "rtsp://192.168.0.28:48010/session", verb: "launch"),
        simulatedClipboardText: "auto pulled"
    )
    let sessionConnector = FakeSessionConnectionClient()
    let clipboard = FakeClipboardClient()
    let runtime = ShadowClientRemoteDesktopRuntime(
    ...
    await waitForClipboardText(clipboard, expected: "auto pulled")
}
