...
struct ShadowClientHostTerminationEvent: Equatable, Sendable {
    ...
    var message: String {
        switch reasonCode {
        case 0x80030023:
            return "Lumen paused or closed the desktop session (0x80030023). This often happens when Windows shows a secure desktop, password prompt, or UAC dialog."
        default:
            return String(format: "Lumen terminated the session (0x%08X).", reasonCode)
        }
    }
}
...
enum ShadowClientHostControlFeedbackCodec {
    ...
    private static func parseRumble(
        ...
        // Lumen-host control_rumble_t packs a 4-byte reserved field before id/low/high.
    ...
    private static func parseAdaptiveTriggers(
        ...
        // Lumen-host control_adaptive_triggers_t:
